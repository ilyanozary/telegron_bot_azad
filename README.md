...
- پنل ادمین تحت وب برای مدیریت کلمات ممنوعه و کانال/گروه اجباری
- ویرایش متن پیام‌های ربات از داخل پنل ادمین
- الزام عضویت در کانال/گروه: اگر کاربر عضو همه موارد تعریف‌شده نباشد، بعد از ۵ پیام، پیام آخرش حذف می‌شود و لینک عضویت می‌گیرد.
...
- تنظیم کانال/گروه اجباری از پنل ادمین انجام می‌شود.
  - هر خط یک کانال یا گروه است؛ کاربر باید عضو همه موارد باشد.
  - برای مورد عمومی، مقدار را مثل `@your_channel` یا `https://t.me/your_group` وارد کنید.
  - بعد از ذخیره، پنل بررسی می‌کند که ربات داخل هر مورد ادمین هست یا نه.
  - اگر ربات ادمین نباشد، پنل پیام هشدار نشان می‌دهد؛ وقتی ادمین شد و دوباره ذخیره/بررسی کنید، پیام موفقیت نمایش داده می‌شود.
  - برای اینکه ربات بتواند عضویت کاربران را چک کند، ربات باید داخل همان کانال/گروه ادمین باشد.
- متن پیام‌های گروه از صفحه «پیام‌های ربات» در پنل قابل تغییر است.
  - `{user}` نام کاربر را وارد می‌کند.
  - `{channel}` لینک یا عنوان کانال/گروه‌های جا مانده را وارد می‌کند.
