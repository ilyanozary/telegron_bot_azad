...
DEFAULT_SETTINGS = {
    ...
    "message_required_channel": (
        "{user} عزیز،\n\n"
        "برای ادامه فعالیت در گروه، لطفاً ابتدا در کانال‌های زیر عضو شوید:\n"
        "{channel}\n\n"
    ...
    "message_link_warning": (
        "{user} عزیز،\n\n"
        "ارسال لینک در گروه مجاز نیست و پیام شما حذف شد. "
        "لطفاً از ارسال لینک‌های تبلیغاتی یا خارج از موضوع خودداری فرمایید."
    ),
    "message_spam_mute": (
        "{user} عزیز،\n\n"
...
def parse_required_channels(raw_channels: str) -> list[str]:
    channels: list[str] = []
    seen: set[str] = set()
    for chunk in raw_channels.replace(",", "\n").splitlines():
        channel = chunk.strip()
